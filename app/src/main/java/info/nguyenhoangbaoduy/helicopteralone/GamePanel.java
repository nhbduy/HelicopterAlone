...
//        bg.setVector(-5);
        ...
        //we can safety start the game loop
    ...
    public void update() {
        if (player.getPlaying()) {
            ...
            for (int i = 0; i < botborder.size(); i++) {
                if (colission(botborder.get(i), player)) {
                    player.setPlaying(false);
                }
            }
            ...
            for (int i = 0; i < topborder.size(); i++) {
                if (colission(topborder.get(i), player)) {
                    player.setPlaying(false);
                }
            }
            ...
            for (int i = 0; i < missiles.size(); i++) {
                ...
                if (colission(missiles.get(i), player)) {
                    missiles.remove(i);
                    player.setPlaying(false);
                    break;
                }
    ...
    public boolean colission(GameObject a, GameObject b) {
    ...
    public void updateTopBorder() {
        ...
        for (int i = 0; i < topborder.size(); i++) {
            ...
            if (topborder.get(i).getX() < -20) {
                ...
                //calcuate topdown which determines the direction the border is moving (up or down)
    ...
    public void drawText(Canvas canvas) {
        ...
        myScore = player.getScore();
        canvas.drawText("DISTANCE: " + myScore, 10, HEIGHT - 10, paint);
        canvas.drawText("BEST: " + bestScore, WIDTH - 150, HEIGHT - 10, paint);
        ...
        if(!player.getPlaying() && newGameCreated && reset) {
            ...
            canvas.drawText("HELICOPTER ALONE", WIDTH / 2 - 150, HEIGHT / 2 - 100, paintTitle);

            //menu game
            Paint paintMenu = new Paint();
            paintMenu.setTextSize(40);
            paintMenu.setTypeface(Typeface.create(Typeface.DEFAULT, Typeface.BOLD_ITALIC));
            canvas.drawText("PRESS TO START", WIDTH / 2 - 50, HEIGHT / 2, paintMenu);
            paintMenu.setTextSize(20);
            canvas.drawText("PRESS AND HOLD TO GO UP", WIDTH / 2 - 50, HEIGHT / 2 + 20, paintMenu);
            canvas.drawText("RELEASE TO GO DOWN", WIDTH / 2 - 50, HEIGHT / 2 + 40, paintMenu);
        }
    }
}
